...
function makeJob(type, params) {
  ...
  const job = {
    ...
    _abort: null, // set to the cancel/teardown function for the job's whole run
  };
...
/**
 * Enqueue a new job.
 * @param {string} type  operation name (deploy/update/delete/alias/cleanup)
...
async function startJob(job) {
  ...
  // Every child an operation spawns registers a cancel fn. Keep ALL of them
  // (not only the latest) so a cancel kills whichever child is actually live.
  const cancels = new Set();
  let cancelReason = null;
  let abortRun;
  const abort = new Promise((_, reject) => { abortRun = reject; });

  // Cancel = kill live children AND reject the runner. The rejection is what
  // matters: a job stuck in a non-child await has no child to kill, and before
  // this the timeout only flipped a flag — the runner never settled, `finally`
  // never ran, and the single concurrency slot was held forever, wedging every
  // job queued behind it until the agent was restarted.
  const cancel = (reason) => {
    if (cancelReason) return;
    cancelReason = reason;
    for (const fn of cancels) { try { fn(reason); } catch {} }
    abortRun(new Error(reason === 'timeout' ? 'operation exceeded time limit' : 'cancelled by request'));
  };
  job._abort = cancel;

  const timeoutMs = config.opTimeouts?.[job.type] || config.jobTimeoutMs;
  ...
  const helpers = {
    ...
    onCancel: (fn) => { cancels.add(fn); },
  };

  try {
    await Promise.race([job._runner(job, helpers), abort]);
    setState(job, STATE.SUCCEEDED);
  } catch (e) {
    if (cancelReason === 'timeout') {
      setState(job, STATE.TIMEOUT, 'operation exceeded time limit');
    } else if (cancelReason === 'cancelled') {
      // Distinct from FAILED so the panel can tell "I stopped it" from "it broke".
      setState(job, STATE.CANCELLED, 'cancelled by request');
    } else {
...
export function cancelJob(id) {
  ...
  // A running job always has _abort (set for its whole lifetime), so running
  // jobs are always cancellable — even when no child process is mid-flight.
  if (typeof job._abort === 'function') {
    job._abort('cancelled');
    return { ok: true };
  }
